...
export default function App() {
    ...
    useEffect(() => {
        // Assuming mockData is defined elsewhere in your file or imported
        if (showAlert && isAnimationComplete && !dialogueData) {
            // @ts-ignore - assuming mockData exists in your environment
            setDialogueData(typeof mockData !== 'undefined' ? mockData : null);
        }
            ...
            <AnimatePresence mode="wait">
                {!gameStarted ? (
                    ...
                    <motion.div
                        ...
                        className="relative flex flex-col md:flex-row h-screen w-full overflow-hidden bg-background-light"
                    >
                        {/* Simulation Map Area */}
                        <main className="relative flex-1 h-full overflow-hidden">
                            ...
                            <motion.div
                                ...
                                className="absolute top-8 left-8 p-3 rounded-lg glass-panel flex flex-col gap-1 shadow-lg z-10 bg-white/80"
                            >
                                    ...
                                    <div className="flex-1 p-5 flex flex-col gap-4">
                                        <div>
                                            ...
                                            {dialogueData ? (
                                                ...
                                                <p className="text-slate-500 text-sm leading-relaxed">Loading transmission...</p>
                                            )}
                                        </div>
                                        <div className="flex gap-3 mt-1">
                                            <button
                                                onClick={handleDismiss}
                                                className="flex-1 bg-primary hover:bg-primary-dark text-white h-11 rounded-lg font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
                                            >
                                                <CheckCircle2 className="w-4 h-4" /> APPROVE
                                            </button>
                                            <button
                                                onClick={handleDismiss}
                                                className="flex-1 bg-white hover:bg-slate-50 border border-slate-200 text-slate-600 h-11 rounded-lg font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
                                            >
